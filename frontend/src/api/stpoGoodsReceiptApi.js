...
async function fetchStpoLive(stpoNumber, lineItem) {
  ...
  const url = `${getStpoFetchPoBaseUrl(creds.environment)}/api/stpo-goods-receipt/stock-transport-order/${normalizedStpo}`;
...
export async function fetchStpoBatchesByMaterial(stpoNumber) {
  ...
  const normalizedStpo = normalizeStpoNumber(stpoNumber);
  const url = `${getStpoFetchPoBaseUrl(creds.environment)}/api/stpo-goods-receipt/stpo-batches/${normalizedStpo}`;
